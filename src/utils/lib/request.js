...
const login = async function (account, password) {
    let res = await request({
        url: 'http://139.199.224.230/oauth/login',
        method: 'POST',
        needLogin: false,
...
const authorize = async function (client_id, state, scope, redirect_uri) {
    /**
     * Oauth授权
     * @returns {Promise<void>}
     * @private
    ...
    async function _oauthLogin() {
        res = await request({
            url: 'http://139.199.224.230/oauth/authorize',
            method: 'GET',
...
const getJson = async function() {
    try {
        ...
        if (res.data.code === '0') {
            ...
            app.globalData.cookieA = { Key, Value }
